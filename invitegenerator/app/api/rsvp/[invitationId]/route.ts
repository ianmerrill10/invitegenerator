...
async function verifyTokenAndGetUserId(token: string): Promise<string | null> {
  ...
  // For now, we'll decode and check the token structure
  try {
...
async function sendOwnerNotification(invitation: any, rsvp: any): Promise<void> {
          ...
          Data: `
            <html>
              <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
                <h2 style="color: #FF6B47;">New RSVP Received!</h2>
                ...
                <p style="margin-top: 20px;">
                  <a href="${process.env.NEXT_PUBLIC_APP_URL}/dashboard/invitations/${invitation.id}/rsvp" 
                     style="background-color: #FF6B47; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px;">
                    View All RSVPs
                  </a>
                </p>
...
async function sendGuestConfirmation(invitation: any, rsvp: any): Promise<void> {
  const attendingText = rsvp.attending === 'yes' 
      ...
      Body: {
        Html: {
          Data: `
            <html>
              <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
                <h2 style="color: #FF6B47;">RSVP Confirmed!</h2>
                <p>Hi ${rsvp.name},</p>
                <p>${attendingText}</p>
                ...
                <p>
                  <a href="${process.env.NEXT_PUBLIC_APP_URL}/i/${invitation.shortId}" 
                     style="color: #FF6B47; text-decoration: underline;">
                    View Invitation
                  </a>
                </p>
                <p style="color: #666; font-size: 12px; margin-top: 30px;">

...
export default function CreateInvitationPage() {
  ...
  const onSubmit = async (data: FormData) => {
    // If using template, redirect to template gallery with the event type filter
    if (creationMethod === "template") {
      // Store form data in sessionStorage for later use
      sessionStorage.setItem("createWizardData", JSON.stringify(data));
      router.push(`/dashboard/templates?category=${data.eventType}`);
      return;
    }

...
interface SanitizedCustomQuestion {
  id: string;
  question: string;
...
function sanitizeCustomQuestions(questions: RawCustomQuestion[] | undefined): SanitizedCustomQuestion[] {
  if (!questions || !Array.isArray(questions)) return [];

  return questions
    .filter((q): q is RawCustomQuestion => q != null && typeof q === 'object')
    .map(q => ({
      id: String(q.id || '').slice(0, 50),
      question: sanitizeText(String(q.question || '').slice(0, 500)),
      type: (['text', 'select', 'checkbox'].includes(q.type || '') ? q.type : 'text') as 'text' | 'select' | 'checkbox',
      required: Boolean(q.required),
      options: Array.isArray(q.options)
        ? q.options.slice(0, 20).map((o: string) => sanitizeText(String(o).slice(0, 100)))
        : undefined,
    }))

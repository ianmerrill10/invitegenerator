...
import { DynamoDBClient, GetItemCommand, QueryCommand, QueryCommandOutput } from '@aws-sdk/client-dynamodb';
import { AttributeValue } from '@aws-sdk/client-dynamodb';
import { unmarshall, marshall } from '@aws-sdk/util-dynamodb';
import { verifyAuth } from "@/lib/auth-server";

interface RSVPExportRecord {
  id: string;
  invitationId: string;
  name: string;
  email: string;
...
async function fetchAllRSVPs(invitationId: string): Promise<RSVPExportRecord[]> {
  const allRsvps: RSVPExportRecord[] = [];
  ...
  while (hasMorePages) {
    ...
    if (response.Items) {
      allRsvps.push(...response.Items.map((item) =>
        unmarshall(item) as RSVPExportRecord
      ));
    }
...
function generateCSV(rsvps: RSVPExportRecord[], customQuestions: CustomQuestion[]): string {
  // Define headers
  const baseHeaders = [
    'Name',
    'Email',
...
function generateXLSX(rsvps: RSVPExportRecord[], customQuestions: CustomQuestion[], title: string): Buffer {
  // Define headers
  const baseHeaders = [
    'Name',
    'Email',
